...
public class Extrapalettor implements Runnable {
  ...
  @Override
  public void run() {
    GraphConfiguration<ColorPoint> conf = this.create_configuration((ColorSpace) p.get("colorspace"), p.getDouble("e0"),
        p.getInteger("nbColours"));
  ...
  private GraphConfiguration<ColorPoint> create_configuration(ColorSpace cs, double p_e0, int nbColours) {
    ...
    e1.addConstraint(new LightnessDistanceFuzzyBinaryConstraint(pts[0], pts[nbColours - 1], 20));
